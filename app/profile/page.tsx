...
import {
    ...
    getMyStatsApiStatsMeGet,
    getUserStatsApiStatsUsersUserIdGet,
    getMyRatingHistoryApiRatingHistoryMeGet,
    getWinProbabilityApiRatingProbabilityGet,
    getRatingProjectionApiRatingProjectionGet,
} from "@/lib/client";
import {
    ChartContainer,
    ChartTooltip,
...
export default function ProfilePage() {
    ...
    useEffect(() => {
        ...
        const loadById = async (id: string, token?: string | null) => {
            ...
            try {
                const reqOptions = token
                    ...
                    : { path: { user_id: id } };
                const resp: unknown =
                    await getUserByIdApiUserUserIdGet(reqOptions);
                const data =
                    resp &&
                    typeof resp === "object" &&
        ...
        const tryLoad = async () => {
            ...
            try {
                ...
                try {
                    const resp: unknown = await (
                        await import("@/lib/client")
                    ).getUserByTokenApiUserTokenGet({
                        headers: { Authorization: `Bearer ${token}` },
                    });
                    const data =
                        resp &&
                ...
                try {
                    ...
                    if (parts.length >= 2) {
                        ...
                        let userId =
                            payload.user_id || payload.id || payload.uid;
                        if (
                            !userId &&
                            payload.sub &&
        ...
        const loadStats = async (token?: string | null, id?: string) => {
            try {
                ...
                if (token && (!id || id === String(storeUser?.id))) {
                    ...
                    resp = await getUserStatsApiStatsUsersUserIdGet({
                        path: { user_id: id },
                    });
                } else {
                    resp = null;
    ...
    const fetchHistory = async (limit = 10) => {
        ...
        if (!token) return;
        try {
            const resp: unknown = await getMyRatingHistoryApiRatingHistoryMeGet(
                {
                    headers: { Authorization: `Bearer ${token}` },
                    query: { limit },
                },
    ...
    const initials = (u?: User | null) => {
        ...
        return (
            res || (u.email ? String(u.email).slice(0, 1).toUpperCase() : "?")
        );
    };
                    ...
                    <div className="grid sm:grid-cols-2 gap-8">
                        <div className="space-y-6">
                            <div className="grid gap-1.5">
                                <Label className="text-[10px] uppercase font-bold tracking-[0.2em] text-muted-foreground/80">
                                    Имя
                                </Label>
                            ...
                            <div className="grid sm:grid-cols-2 gap-4">
                                <div className="p-4 border rounded">
                                    ...
                                    <div className="text-xl font-bold">
                                        {stats.pvp?.wins ?? 0} /{" "}
                                        {stats.pvp?.matches ?? 0} побед
                                    </div>
                                    <div className="text-sm text-muted-foreground">
                                        Побед: {stats.pvp?.wins ?? 0} •
                                        Поражений: {stats.pvp?.losses ?? 0} •
                                        Ничьих: {stats.pvp?.draws ?? 0}
                                    </div>
                                </div>
                                ...
                                <div className="p-4 border rounded">
                                    ...
                                    <div className="text-xl font-bold">
                                        {stats.training?.correct ?? 0} /{" "}
                                        {stats.training?.attempts ?? 0}
                                    </div>
                                    <div className="text-sm text-muted-foreground">
                                        Точность:{" "}
                                        {stats.training?.accuracy_pct ?? 0}%
                                    </div>
                                </div>
                            </div>
                        </div>
                                ...
                                >
                                    {(() => {
                                        ...
                                        const data = items.map(
                                            (it: HistoryItem, idx: number) => ({
                                                date: `#${idx + 1}`,
                                                rating:
                                                    (it.my_rating_before ?? 0) +
                                                    (it.my_rating_delta ?? 0),
                                            }),
                                        );
                                        ...
                                        const padding = Math.max(
                                            Math.round(span * 0.12),
                                            25,
                                        );

                                        const extraTop = Math.max(
                                            30,
                                            Math.round(span * 0.18),
                                            Math.round(maxRating * 0.02),
                                        );
                                        domainMin = Math.max(
                                            0,
                                            Math.floor(
                                                (domainMin - padding) / 10,
                                            ) * 10,
                                        );
                                        domainMax =
                                            Math.ceil(
                            ...
                            <div className="space-y-2">
                                {(history?.items ?? []).map(
                                    (it: HistoryItem) => (
                                        <div
                                            key={it.match_id}
                                            className="p-3 border rounded flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-4"
                                        >
                                            <div className="min-w-0 sm:flex-1">
                                                <div className="text-sm truncate">
                                                    Против:{" "}
                                                    {it.opponent?.first_name &&
                                                    it.opponent?.last_name
                                                        ? `${it.opponent.first_name} ${it.opponent.last_name}`
                                                        : (it.opponent?.email ??
                                                          "—")}
                                                </div>
                                            </div>

                                            <div className="text-xs sm:text-sm text-muted-foreground sm:text-center shrink-0">
                                                <span>
                                                    Рейтинг до:{" "}
                                                    {it.my_rating_before}
                                                </span>{" "}
                                                <span>
                                                    • Δ {it.my_rating_delta}
                                                </span>
                                            </div>

                                            <div className="shrink-0 sm:text-right text-sm text-muted-foreground whitespace-nowrap">
                                                {it.result || it.state}
                                            </div>
                                        </div>
                                    ),
